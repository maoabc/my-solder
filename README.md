...
这个项目从硬件焊接到固件刷机，都设计得简单易上手。总成本低（核心部件如C210头可在闲鱼买到10元包邮），但需要一些电子和3D打印基础。整个过程分硬件、3D外壳和固件三步。**注意**：全程避免短路，测试时用万用表检查。

### 1. 准备材料和工具
- **核心电子部件**（详见hardware目录的BOM文件）：
  - STM32微控制器（主控）。
  - TPS62933稳压器（提供3.3V电源）。
  - MCP1415驱动器（C210推荐版，用于高频驱动减少噪音；电阻分压版可选但有噪音）。
  - 姿态传感器（用于自动检测闲置休眠）。
  - 电阻、电容、Type-C接口、按键小板等外围。
...
**打印步骤**：
1. 打开.scad文件，输入你的C210/T12头测量值（直径、长度），生成STL。
2. 打印两部分：外壳主体 + 按键模块&顶部盖（一体打印）。
4. 组装：将PCB塞入主体，按键盖扣上。确保加热芯插入顺畅，无松动。

**提示**：如果没打印机，导出STL后找服务打印。测试时，确保外壳不导电，避免短路。
